...
class Solver:
    ...
    def serialize_checkpoint(self) -> dict[str, Any]:
        ...
        for entry_id, entry in self.grid.entries.items():
            ...
            if entry.placement:
                placement_data = {
                    "entry_id": entry.placement.entry_id,
                    "candidate": {
                        "answer": entry.placement.candidate.answer,
                        "search_level": entry.placement.candidate.search_level,
                        "llm_confidence": entry.placement.candidate.llm_confidence,
                        "logprob_confidence": entry.placement.candidate.logprob_confidence,
                        "penalty": entry.placement.candidate.penalty,
                    },
                    "search_level": entry.placement.search_level,
                    "pattern": entry.placement.pattern,
                    "selection_score": entry.placement.selection_score,
                    "is_fallback": entry.placement.is_fallback,
                }
            
            entries_state[entry_id] = {
                "candidates": candidates_list,
                "pattern_levels": dict(entry._pattern_levels),
    ...
    def restore_from_checkpoint(self, checkpoint_data: dict[str, Any]) -> None:
        ...
        for entry_id, entry_data in entries_state.items():
            ...
            for cand_data in candidates:
                candidate = Candidate(
                    entry_id=entry_id,
                    answer=cand_data["answer"],
                    search_level=cand_data.get("search_level", 0),
                    llm_confidence=cand_data.get("llm_confidence", 0.0),
                    logprob_confidence=cand_data.get("logprob_confidence", 0.0),
                    penalty=cand_data.get("penalty", 0.0),
                )
                entry._candidates[candidate.answer] = candidate
            
            # Restore pattern levels
            entry._pattern_levels = dict(entry_data.get("pattern_levels", {}))
            
            # Restore placement
            placement_data = entry_data.get("placement")
            if placement_data:
                # Reconstruct the candidate from placement data
                cand_data = placement_data["candidate"]
                candidate = Candidate(
                    entry_id=placement_data["entry_id"],
                    answer=cand_data["answer"],
                    search_level=cand_data.get("search_level", 0),
                    llm_confidence=cand_data.get("llm_confidence", 0.0),
                    logprob_confidence=cand_data.get("logprob_confidence", 0.0),
                ...
                entry.placement = Placement(
                    entry_id=placement_data["entry_id"],
                    candidate=candidate,
                    search_level=placement_data.get("search_level", 0),
                    pattern=placement_data["pattern"],
                    selection_score=placement_data["selection_score"],
                    is_fallback=placement_data.get("is_fallback", False),
                )
            else:
                entry.placement = None
            
            # Restore other state
            entry.backtracks = entry_data.get("backtracks", 0)
            entry.used_fallback = entry_data.get("used_fallback", False)
