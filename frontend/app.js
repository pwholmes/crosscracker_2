...
function setState(newState, customMessage = null, source = null) {
	...
	// Hide progress bar when transitioning away from progress-bearing states
	if (newState !== AppState.INITIALIZING && newState !== AppState.STEPPING && newState !== AppState.LOADING) {
...
function processMessage(data) {
	...
	if (data.type === 'state') {
		...
		log("ALL DONE")
		// All state transitions and UI updates are controlled by AppState.
		// Just ensure we're in a stable state and don't override completed states.
		if (currentState === AppState.PLAYING) {
			// Stay in playing state
			setState(AppState.PLAYING, null, 'ws:progress_done');
		} else if (currentState === AppState.REPLAY_PLAYING || currentState === AppState.REPLAY_IDLE) {
			// Stay in replay state
		} else if (currentState === AppState.COMPLETED_SOLVED || currentState === AppState.COMPLETED_FAILED) {
			// Preserve terminal states - setState will hide progress bar
		} else if (currentState === AppState.STEPPING) {
			// Step is done - transition to idle (setState will hide progress bar)
			setState(AppState.IDLE, null, 'ws:progress_done');
		}
		// Other states manage their own transitions via state handlers
	}
}
...
async function postAction(path) {
	try {
		...
		if (path === '/step' && currentState === AppState.STEPPING) {
			// After a step completes, return to idle (unless it solved/failed)
			// This is a safety net in case progress_done didn't arrive
